...
def calculate_trading_probability(data):
    data_range = pd.date_range(
        start=data["기준날짜"].min(), end=data["기준날짜"].max(), freq="B"
    )
    ...
    return trading_probability.reset_index().rename(
        columns={"기준날짜": "매수진행확률"}
    )
...
if st.sidebar.button("데이터 가져오기"):
    ...
    if uploaded_file is not None:
        ...
        with col2:
            bar_chart = px.bar(
                bar_data, x="종목명", y="수익률", title="종목별 평균 수익률"
            )
            st.plotly_chart(bar_chart, use_container_width=True)

        with col3:
            trading_bar_chart = px.bar(
                trading_probability_data,
                x="종목명",
                y="매수진행확률",
                title="종목별 일일 매수 확률",
            )
