...
class MyWindow(QMainWindow, form_class):
    ...
    def start_trading(self):
        ...
        self.bought_list = {
            code: today
            for code, buy_date in self.bought_list.items()
            if buy_date == today
        }
    ...
    def trade_stocks(self):
        ...
        for code in codes:
            if code.strip() and (
                code.strip() not in self.bought_list
                or self.bought_list[code.strip()] != today
            ):
    ...
    def sell_all_stocks(self):
        ...
        stocks_info = self.kiwoom.block_request(
            "opw00018",
            계좌번호=account_number,
            비밀번호="",
            비밀번호입력매체구분="00",
        ...
        # Additional code from <코드2>
        # 계좌 정보와 로그인 부분은 중복 제거됨
